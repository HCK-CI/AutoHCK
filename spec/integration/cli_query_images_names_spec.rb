...
describe 'CLI and Project#prepare query mode (images-names)', :integration do
  ...
  let(:config_fixture) { File.expand_path('../fixtures/minimal_config.json', __dir__) }
  ...
  before do
    config_data = JSON.parse(File.read(config_fixture))
    allow(AutoHCK::Config).to receive(:read).and_return(config_data)
