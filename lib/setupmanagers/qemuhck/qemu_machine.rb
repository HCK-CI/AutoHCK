...
module AutoHCK
  ...
  class QemuMachine
    ...
    def run_vm
      dump_config
      Thread.new do
        run_cmd([replace_string_recursive(dirty_command.join(' '), full_replacement_list)])
      end
      sleep 5
      @pid = retrieve_pid
    ...
    def run_pre_start_commands
      @pre_start_commands.each do |cmd|
        run_cmd([replace_string_recursive(cmd, full_replacement_list)])
      end
    end

    def run_post_stop_commands
      @post_stop_commands.each do |cmd|
        run_cmd_no_fail([replace_string_recursive(cmd, full_replacement_list)])
      end
    end
