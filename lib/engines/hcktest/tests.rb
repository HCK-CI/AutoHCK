...
module AutoHCK
  ...
  class Tests
    ...
    def run_test_commands(test, type)
      select_test_config(test['name'], type).each do |commands|
        guest_cmd = commands.guest_run
        description = commands.desc
        if guest_cmd
          run_command_on_client(@client, guest_cmd, description)
          run_command_on_client(@support, guest_cmd, description) if support_needed?(test)
        end

        next unless commands.host_run

        @logger.info("Running command (#{description}) on host")
        run_cmd(commands.host_run)
      end
    end
